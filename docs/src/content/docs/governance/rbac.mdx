...
Role-Based Access Control is an Enterprise Edition feature. It is included in [Hosted SaaS](https://terrateam.io) and self-hosted Enterprise deployments. Setting `access_control.enabled: true` on the Open Source Edition is not permitted — Terrateam rejects the configuration and posts a comment on the pull request. See [Editions](/editions/).
:::
