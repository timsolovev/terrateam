...
`ci_config_update` and `terrateam_config_update` are available in the [Enterprise edition](/editions/).
:::
...
For maximum security, combine all four layers. Layers 1 and 2 are available to all users. Layer 3 requires [Enterprise](/editions/). Layer 4 is available to all users and provides the strongest trust policy enforcement.
