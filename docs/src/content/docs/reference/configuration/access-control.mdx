...
Enabling access control (`access_control.enabled: true`) is an Enterprise Edition feature. It is included in [Hosted SaaS](https://terrateam.io) and self-hosted Enterprise deployments. On the Open Source Edition, access control is permanently disabled — setting `enabled: true` is not permitted and Terrateam will reject the configuration. See [Editions](/editions/).
:::
