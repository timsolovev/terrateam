...
The API access-token endpoints are part of the Enterprise Edition. They are available on [Hosted SaaS](https://terrateam.io) and self-hosted Enterprise deployments. The Open Source Edition does not register the `/api/v1/{vcs}/access-token` routes. See [Editions](/editions/).
:::
